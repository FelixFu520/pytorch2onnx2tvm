...
int main(void) {
  tvm::runtime::Module mod_dylib =
      tvm::runtime::Module::LoadFromFile("./mobilenet.so");

  std::ifstream json_in("./mobilenet.json", std::ios::in);
  std::string json_data((std::istreambuf_iterator<char>(json_in)), std::istreambuf_iterator<char>());
  json_in.close();
  
  std::ifstream params_in("./mobilenet.params", std::ios::binary);
  ...
  std::ifstream data_fin("../plane.bin", std::ios::binary);
